...
\author[GGVY -- slides on \url{https://github.com/gvegayon/faculty-talk}]{George G Vega Yon}
...
%	\frametitle{George G. Vega Yon}
	...
	\large My work sits on the intersection between...\bigskip\normalsize
	

\byside{.32}{Statistics}{\bigskip\includegraphics[width=.8\linewidth]{mcmc.png}}
\byside{.32}{Computer Science}{\bigskip\includegraphics[width=.8\linewidth]{time.png}}
\byside{.32}{Complex Systems}{\bigskip\includegraphics[width=.8\linewidth]{complex.png}}
...
\begin{minipage}[m]{.45\linewidth}
	\begin{figure}
		\includegraphics[width=1\linewidth]{roc-auc-aphylo-sifter.eps}
	\end{figure}
\end{minipage}\hfill
\begin{minipage}[m]{.45\linewidth}
	Comparing aphylo vs SIFTER (the current state-of-the-art phylo-based evolutionary model) on 147 proteins:
\begin{itemize}
	\item[] \shadowbox{Fast} SIFTER took about 110 minutes to calculate the posterior probabilities, aphylo took 1 second.
	\item[] \shadowbox{Accurate} aphylo reported higher accuracy levels in LOO cross-validation (0.72 vs 0.60 AUC).
\end{itemize}
\end{minipage}
...
%	}\hfill\pause
%	\byside{.24}{Competitive}{Matches state-of-the-art phylo-based prediction models with $\sim$ 0.75 AUC.}
%	\vfill
...
\begin{minipage}[t]{.49\linewidth}
	\begin{center}
		\shadowbox{Ground truth}
	\end{center}
\begin{figure}[p]
	\centering
	\includegraphics[width=.9\linewidth]{time-diagram-actual.png}
%	\caption{Sequence of actions in an event. Several factors influence officers' actions: Who takes action first, what suspects do, and how their peers react.}
%	\label{fig:my_label}
\end{figure}
\begin{itemize}
	\item Sequence of actions
	\item Subjects' Behavior
	\item Peer influence
	\item Other situational factors
\end{itemize}
\end{minipage}\hfill
\begin{minipage}[t]{.49\linewidth}
	\begin{center}
...
\begin{figure}
	\centering
	\includegraphics[width=.9\linewidth]{time-diagram-observed.png}
%	\caption{Observed data. Of the many parts that give origin to individuals' actions, we only observe a handful. Furthermore, we do not know the order in which things occurred, how the subjects reacted, and whether other external factors played out.}
%	\label{fig:my_label}
\end{figure}
\begin{itemize}
	\item We only observe draw/no draw
	\item We are unaware of subjects' behavior
	\item Don't know if there's contagion/anti-contagion
	\item Other factors are unobserved as well
\end{itemize}
\end{minipage}
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		\item Assuming no peer effect/influence/contagion
		\begin{equation*}\label{eq:baseline}
			\Prcond{Y_{it} = 1}{\Covar_{it},\Latent_{t}} = \logit{\covar_{it}\beta + \latent_t}
		\end{equation*}
		Yet, $\latent_t$ (subjects' behavior, etc.) is unobserved.
		\item Conditioning on $\sum_iy_{it}$, we can instead estimate the following
		\begin{equation*}\label{eq:clogit}
			\Prcond{\mathbf{Y}_t = \mathbf{y}_t}{\sum_{i}Y_{it} = k,\Covar_{it}} = \frac{%
				\exp{\sum_{i}y_{it}\covar_{it}\beta}
			}{%
				\sum_{\mathbf{y}' \in \pset{k}}\exp{\sum_{i}y_{it}'\covar_{it}\beta}
			}
		\end{equation*}
...
\begin{frame}[t]
	\usebeamertemplate{section intro}{}{}
	\textcolor{uscgold}{
		\Large {\bf Scientific Software Development} 
	}
\end{frame}
