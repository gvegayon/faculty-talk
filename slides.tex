...
\date{Job Talk at Universidad Adolfo Ibáñez\\December 10, 2020}
...
%	\frametitle{George G. Vega Yon}
	...
	\Large My work sits on the intersection between...\normalsize
	

\uncover<2->{\byside{.32}{Statistics}{\includegraphics[width=.8\linewidth]{mcmc.png}}}
\uncover<3->{\byside{.32}{Computer Science}{\includegraphics[width=.8\linewidth]{time.png}}}
\uncover<4->{\byside{.32}{Complex Systems}{\includegraphics[width=.8\linewidth]{complex.png}}}
...
(\small 2nd round of review at \textit{PLOS Computational Biology})
}
\end{frame}
...
\bigskip\uncover<4>{\raggedleft\Large ... Phylogenetic trees can help us answer this question. \normalsize}
...
\item The GO project has $\sim$ 44,400 validated terms \pause \hyperlink{go-functions-types}{\beamergotobutton{more}}, $\sim$ 7.9M annotations\pause on $\sim$ 4,600 species.\pause
...
\item Yet, less than 10\% of those annotations are based on experimental evidence.\pause % 40610
...
%			\oinclude{6}{example-trees-good1-loo-annotated.pdf}
		...
		\mode<handout>{
			\centering
			\includegraphics[width=\tmpwdth, clip, trim={0 0 0 2cm}]{example-trees-good1-loo-annotated-parts-1.pdf}
		}
...
\begin{frame}
	\frametitle{Computational Complexity: aphylo}
	...
	\begin{itemize}
		...
		\uncover<7->{$\implies$ Fast} \\
		\uncover<8->{$\implies$ Efficient use of memory (data structures)}
	\end{itemize}
	\end{minipage}\hfill
...
\begin{minipage}[m]{.28\linewidth}
	...
	\fbox{\uncover<1->{{\large $\checkmark$}}} Higher convergence rate.\\
	\fbox{\uncover<1->{{\large $\checkmark$}}} Lower type-I error rate.\\
	\fbox{\uncover<1->{{\large $\checkmark$}}} Smaller bias.\\
	\fbox{\uncover<1->{{\large $\checkmark$}}} More power.\\
	\fbox{\uncover<1->{{\large $\checkmark$}}} Faster.\\
	\fbox{\uncover<1->{{\large $\checkmark$}}} More flexible.
	
	\uncover<3->{\includegraphics[width=.7\linewidth]{cran-downloads-ergmito.pdf}}
\end{minipage}
\hfill
\begin{minipage}[m]{.34\linewidth}
	\centering
	\uncover<2->{\includegraphics[width=.99\linewidth]{ergmito-page-cran.png}}
\end{minipage}
\hfill
\begin{minipage}[m]{.34\linewidth}
	\centering
	\uncover<2->{\includegraphics[width=.99\linewidth]{ergmito-page-github.png}}
\end{minipage}
...
\begin{frame}
	...
	\centering
	\uncover<4->{\includegraphics[width=1\linewidth]{ergm-computing.pdf}}
	\end{minipage}
...
\begin{frame}
	...
	\begin{figure}[tb]
		\centering
		\input{fig-struct-test/struct-test.pdf_tex}
...
\begin{frame}
	\Large Keep building bridges...
	
	\mode<beamer>{
		\begin{figure}
					\centering
	\includegraphics<1>[width=.65\linewidth]{coauthors0.png}
	\includegraphics<2->[width=.65\linewidth]{coauthors.png}
	\end{figure}
}
\mode<handout>{
	\begin{figure}
		\centering
	\includegraphics[width=.65\linewidth]{coauthors.png}
	\end{figure}
}
