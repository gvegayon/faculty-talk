...
\begin{frame}[c,label=aphylo-prob-diagram]
	\large What is the probability that the human gene \textbf{XYZ} has function \textbf{F}, given what we know about that function for other \textit{related} species?\normalsize
...
\begin{frame}[t, label=aphylo-good]
	\frametitle{\textbf{aphylo}: An evolutionary model of gene functions}
...
\begin{frame}[c,label=current-research]
\frametitle{Current Research}

\byside{.33}{Biology}{Extension of gene functional-evolution model using the ERGMs framework\\
\hyperlink{aphylo-current}{\beamergotobutton{more}}}\hfill
\byside{.33}{Criminology}{The role of social networks on police use of force}\hfill
\byside{.33}{Sociology}{Using ERGMitos to model political discussion networks in Romania}
