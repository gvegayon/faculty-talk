...
(\small \sout{2nd round of review} Accepted at \textit{PLOS Computational Biology})
}
\end{frame}
...
\begin{frame}[c,label=geneontology]
	...
	\begin{minipage}[m]{.66\linewidth}
		...
		\end{figure}
	\begin{itemize}
		\item $\sim$ 15,000 evolutionary trees		
		\item $\sim$ 8 million annotations
		\item $\sim$ 600 thousand on human genes
		\item $\sim$ $<$ 10\% are based on experimental evidence
	\end{itemize}
	\end{minipage}
\vfill \hfill
\Large ...Improving our knowledge on this is key to biomedical research\normalsize\\
...
\begin{frame}[c]
	...
	\end{minipage}\hfill
	\begin{minipage}[t]{.30\linewidth}\centering
		\shadowbox{Scalability}\\
		Simplicity and computational efficiency
		allow making \textbf{inference and prediction fast}\\
		(1 second vs 2 hours)
	\end{minipage}\hfill
	\begin{minipage}[t]{.30\linewidth}\centering
		\shadowbox{Accuracy}\\
		\textbf{aphylo} outperforms state-of-the-art
		phylo-models\\
		(0.72 vs 0.60 AUC)
	\end{minipage}
\end{frame}
...
\end{minipage}\hfill
	\begin{minipage}[t]{.40\linewidth}		
		...
		\vfill 5 parameters}
	\end{minipage}
\uncover<4->{\vfill\hfill\hyperlink{aphylo-ergm-table}{\beamergotobutton{numeric example}}}
\end{frame}
